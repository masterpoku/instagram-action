...
def comment(url, comment_text):
    try:
        media_id = cl.media_pk_from_url(url)
        print(f"\n\033[36m Comment: {comment_text}")
 ...
 ·——————————————————————————————————————————————————————————————————————————
	...
	if opt == 1:
		...
		conexit()
            ...
            url = input("\n\033[33m url: ")
            comment_text = input(" comment: ")  # Renamed variable to avoid conflict
            comment(url, comment_text)  # Call the comment function
            conexit()  # Ensure conexit is defined elsewhere
	elif opt == 00:
			time.sleep(0.5)
			exit = "\n\033[32m [+] Thank You For Using !!"
			typewriter(exit)
